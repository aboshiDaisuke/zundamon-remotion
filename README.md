...
2. **Node.js ＆ Remotionの準備**
   - **一番かんたん（AI丸投げ）**: チャット欄に `「Node.jsの導入から環境セットアップまで全部やって！」` と投げるだけで、AIがOSに合わせて全自動で構築します。
   - **手動の場合**: [Node.js公式サイト](https://nodejs.org/)から緑のボタン「LTS（推奨版）」をダウンロードして「次へ」でインストールするだけでOKです。
   - **Remotionはどこで発動するの？**: 編集ソフトを起動する必要はありません！フルHD・30fps・台本尺などの設定はすべて組み込み済み。AIが裏で `npm run render` コマンドを叩くことで自動発動し、動画を1コマずつレンダリングして完成させます。
...
## 🎭 キャラクター・声の変更（無限のバージョン量産）

- **声の変更**: VOICEVOXのキャラクターIDを変えるだけで、春日部つむぎ、冥鳴ひまり、玄野武宏など好きな声質に変更できます。
- **立ち絵の変更**: お好みの立ち絵PSDファイルを置くだけで、自動分解・リップシンクがそのまま動作します。
- **ジャンル展開**: IT用語解説、歴史解説、商品レビューなど、テーマに合わせてオリジナルの解説シリーズをいくらでも作れます。

---

## 🔍 素材探しもAIに丸投げ ＆ 安全運用ルール

- **チャットで依頼するだけ**: 「このテーマに合う無料の背景探して」「テンポの良いBGMを提案して」とチャットでつぶやくだけで、AIがフリー素材をリサーチして配置まで行います。
- **⚠️ スクレイピング注意**: 相手先サーバーに負荷をかける過剰なスクレイピングは避け、規約が明確な商用フリー素材サイト（みんちりえ、DOVA-SYNDROME、いらすとや等）から安全・合法に取得するルールを徹底しています。

---

## 🎨 画像生成できるAIなら、この中で直接生成OK！

- **素材サイトを探す手間ゼロ**: Antigravity、ChatGPT Plus（DALL-E）、画像生成対応ツールを使っているなら、外部サイトから素材を探す必要すらありません。
- **チャットで一言つぶやくだけ**: 「徳島ラーメンの美味しそうなイラストを生成して中央に置いて」「近未来都市の背景イラストを作って」と指示するだけで、AIがプロジェクト内で直接画像を生成し、Remotionの中央カード（高さ約385px）や背景に自動セットしてくれます。
